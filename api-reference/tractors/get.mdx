---
title: 'Get Tractor'
description: 'Retrieve a specific tractor by ID, get available tractors, or search with advanced filters'
---
...
const response = await fetch(
  'http://localhost:4000/api/tractors/available?limit=5&offset=0',
  {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
...
const data = await response.json();
console.log(`Found ${data.data.length} available tractors`);
```
</CodeGroup>
...
<ParamField query="minPower" type="number">
...
<ParamField query="tractionType" type="string">
...
<CodeGroup>
```bash Search by Power Range
curl -X GET "http://localhost:4000/api/tractors/search?minPower=100&maxPower=200" \
  -H "Content-Type: application/json"
```

```bash Search by Brand and Type
curl -X GET "http://localhost:4000/api/tractors/search?brand=John+Deere&tractionType=4x4" \
  -H "Content-Type: application/json"
```

```javascript JavaScript
const response = await fetch(
  'http://localhost:4000/api/tractors/search?minPower=150&tractionType=4x4',
  {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
...
const data = await response.json();
console.log(`Found ${data.pagination.total} tractors matching criteria`);
```
</CodeGroup>
